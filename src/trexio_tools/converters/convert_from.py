...
def run_resultsFile(filename, trexio_filename, motype=None, back_end=trexio.TREXIO_HDF5):
    ...
    # If there are multiple states, there will be those many number of hdf5 files stored
    for s in range(res.num_states):
        # Open the TREXIO file for writing. Create trexio file for each state
        trexio_filename = f"{trexio_basename}_state_{s}.hdf5"
    ...
    #        trexio.write_mo_occupation(trexio_file, OccNum)
        ...
        trexio_file.close()
        print("Conversion to TREXIO format has been completed for the state ", s, " in the file ", trexio_filename)
...
def run(trexio_filename, filename, filetype, back_end, motype=None):
    ...
    if back_end == trexio.TREXIO_HDF5:
        if existing_file:
            print(f"TREXIO files matching {trexio_basename}_state_*.hdf5 already exist and will be removed before conversion.")
    ...
    if filetype.lower() == "gaussian":
        run_resultsFile(filename, trexio_filename, motype, back_end=back_end)
